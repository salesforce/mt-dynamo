...
public class CachingAmazonDynamoDbStreams extends DelegatingAmazonDynamoDbStreams {
    ...
    private final Cache<StreamShardId, Optional<CachingShardIterator>> trimHorizonCache; // Optional.empty == SHARD_END
    ...
    CachingAmazonDynamoDbStreams(AmazonDynamoDBStreams amazonDynamoDbStreams,
                                 ...
                                 Cache<StreamShardId, Optional<CachingShardIterator>> trimHorizonCache) {
    ...
    public GetShardIteratorResult getShardIterator(GetShardIteratorRequest request) {
        ...
        switch (ShardIteratorType.fromValue(request.getShardIteratorType())) {
            case TRIM_HORIZON:
                 ...
                 */
                Optional<CachingShardIterator> cachedIterator =
                    trimHorizonCache.getIfPresent(new StreamShardId(request.getStreamArn(), request.getShardId()));
                if (cachedIterator != null && cachedIterator.isPresent()) {
                    return new GetShardIteratorResult().withShardIterator(cachedIterator.get().toExternalString());
                } else {
    ...
    public GetRecordsResult getRecords(GetRecordsRequest request) {
        return getRecordsTime.record(() -> {
            ...
            final CachingShardIterator iterator;
            if (parsed.type == TRIM_HORIZON) {
                Optional<CachingShardIterator> cached = trimHorizonCache.getIfPresent(parsed.getStreamShardId());
                ...
                if (cached == null) {
                    ...
                    return new GetRecordsResult().withRecords(Collections.emptyList()); // empty shard
                }
            } else {
                iterator = parsed;
            }
            ...
            if (positionOpt.isPresent()) {
                ...
                // if the iterator has an absolute position, check the record cache
                List<Record> cachedRecords = recordCache.getRecords(position, limit);
                if (cachedRecords.isEmpty()) {
                    // cache miss: try to acquire load lock for position (to avoid concurrent loads). Limit the max
                    // wait time, since we are accessing an external resource while holding the lock.
                    ...
                    if (getRecordsLoadLockWaitTime.record(() -> {
                        ...
                        try {
                            cachedRecords = recordCache.getRecords(position, limit);
                            result = cachedRecords.isEmpty()
                                ? loadRecords(iterator, limit) // still nothing cached: load from the shard
                                : iterator.nextResult(cachedRecords); // otherwise return what was concurrently loaded
                        } finally {
                            lock.unlock();
                        }
                    } else {
                        // failed to get load lock: check if caches have been updated, but don't attempt to load.
                        ...
                        if (cachedRecords.isEmpty()) {
                            ...
                            if (LOG.isWarnEnabled()) {
                                LOG.warn("Failed to obtain getRecords lock or cached records for iterator {}.",
                                    iterator);
                            }
                    ...
                    // (partial) cache hit: return records from cache
            ...
            if (loadedRecords.isEmpty()) {
                ...
                if (loadedNextIterator == null) {
                    if (iterator.getType() == TRIM_HORIZON) {
                        trimHorizonCache.put(iterator.getStreamShardId(), Optional.empty());
                    }
                    nextIterator = null;
                } else {
                ...
                if (iterator.getType() == TRIM_HORIZON) {
                    trimHorizonCache.put(iterator.getStreamShardId(),
                        Optional.of(iterator.resolvePositionIterator(loadedRecords)));
                }
            }

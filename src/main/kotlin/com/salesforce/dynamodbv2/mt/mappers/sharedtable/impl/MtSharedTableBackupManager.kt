...
open class MtSharedTableBackupManager(
    ...
    override fun restoreTenantTableBackup(
        ...
        val createTableReq: CreateTableRequest = getTenantTableMetadata(restoreMtBackupRequest.backupArn,
                restoreMtBackupRequest.sourceTenantTableBackup)
        mtContext.withContext(restoreMtBackupRequest.targetTenantTable.tenantName) {
            sharedTableMtDynamo.createTable(createTableReq.withTableName(restoreMtBackupRequest.targetTableName))
        }

        // restore tenant-table data
        val backupFileKeys = getBackupFileKeys(restoreMtBackupRequest.backupArn,
                restoreMtBackupRequest.sourceTenantTableBackup)
        for (fileName in backupFileKeys) {
            ...
            for (row in rowsToInsert) {
                mtContext.withContext(restoreMtBackupRequest.targetTenantTable.tenantName) {
                    sharedTableMtDynamo.putItem(restoreMtBackupRequest.targetTenantTable.virtualTableName, row.attributeMap)
                }
            }
        }
        logger.info("${restoreMtBackupRequest.backupArn}: Finished restoring ${restoreMtBackupRequest.sourceTenantTableBackup}" +
                " to ${restoreMtBackupRequest.targetTenantTable} in ${System.currentTimeMillis() - startTime} ms")

        return TenantRestoreMetadata(restoreMtBackupRequest.backupArn,
                Status.COMPLETE,
                restoreMtBackupRequest.targetTenantTable.tenantName,
                restoreMtBackupRequest.targetTenantTable.virtualTableName)
    }
